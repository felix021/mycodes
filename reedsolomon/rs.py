...
@2013.04.22 this piece of code now failes to correct errors. Maybe someday I'll fix it.
...
.. note::
   ...
   The algorithm itself can handle messages up to self.n bytes, including the ECC bytes. The
...
::
    ...
    >>> rs = RSCodec(255, 245) #RS(n, k), nsym = (n - k) = 10
    >>> rs.encode([1,2,3,4])
    bytearray(b'\x01\x02\x03\x04`\x1c&\xcf\xe5\xfa\xdb\xb4\xb1"')
    >>> rs.encode(b'hello world')
    bytearray(b'hello world&\x19.\xb2>\xb8\xc6})\xac')
...
class RSCodec(object):
    ...
    def rs_calc_syndromes(self, msg, nsym):
        return [self.gf_poly_eval(msg, self.gf_exp[i]) for i in range(1, nsym + 1)]
